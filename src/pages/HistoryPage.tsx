...
export function HistoryPage() {
  const { history, deleteFood, clearHistory } = useNutrition();
  ...
  const handleDelete = (id: string, name: string) => {
    deleteFood(id);
    showToast(`${name} removed from history.`, 'info');
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-display font-bold text-2xl sm:text-3xl text-slate-900 dark:text-white">History Log</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">{history.length} items tracked in total.</p>
        </div>
        {history.length > 0 && (
          <button
      ...
      <div className="glass rounded-2xl p-4 space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <FilterPill icon={Calendar} label="Day" value={dayFilter} options={[{ v: 'all', l: 'All time' }, { v: 'today', l: 'Today' }, { v: 'week', l: 'This week' }]} onChange={(v) => setDayFilter(v as DayFilter)} />
          <FilterPill icon={Heart} label="Health" value={healthFilter} options={[{ v: 'all', l: 'All' }, { v: 'healthy', l: 'Healthy' }, { v: 'unhealthy', l: 'Indulgent' }]} onChange={(v) => setHealthFilter(v as HealthFilter)} />
          <FilterPill icon={Beef} label="Protein" value={proteinFilter} options={[{ v: 'all', l: 'All' }, { v: 'high', l: 'High protein' }]} onChange={(v) => setProteinFilter(v as ProteinFilter)} />
        </div>
      </div>
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${item.isHealthy ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300' : 'bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300'}`}>
                    {item.isHealthy ? 'Healthy' : 'Indulgent'} · {item.healthScore}
                  </span>
                </div>
      ...
      {confirmClear && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-fade-in" onClick={() => setConfirmClear(false)}>
          <div className="glass-strong rounded-2xl p-6 max-w-sm w-full animate-scale-in" onClick={(e) => e.stopPropagation()}>
            ...
            <h3 className="font-display font-bold text-lg text-slate-900 dark:text-white text-center mb-2">Clear all history?</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 text-center mb-6">This will permanently delete all {history.length} tracked items. This action cannot be undone.</p>
